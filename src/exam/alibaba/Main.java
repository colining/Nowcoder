    ...
    public static List<String> calculateUnilateral(List<UnilateralLine> lineList) {
        ...
        for (int i = 0; i < lineList.size(); i++) {
            ...
            for (int j = i+1; j <lineList.size(); j++) {
                if (unilateralLine.getSCen().equals(lineList.get(j).getECen()) && unilateralLine.getECen().equals(lineList.get(j).getSCen()) && unilateralLine.getTType() == lineList.get(j).getTType()) {
                    result.add(unilateralLine.getId() + "+" + lineList.get(j).getId());
                }
